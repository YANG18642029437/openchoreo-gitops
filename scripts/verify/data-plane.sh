...
runtime=platform/openchoreo/data-plane-runtime
rbac="$runtime/crossplane-rbac.yaml"
...
test -f "$runtime/kustomization.yaml" || { echo "missing Data Plane runtime kustomization" >&2; exit 1; }
test -f "$rbac" || { echo "missing Crossplane Data Plane RBAC" >&2; exit 1; }
...
grep -q 'path: platform/openchoreo/data-plane-runtime' "$application"
...
grep -q 'database.openchoreo.io' "$rbac"
grep -q 'xpostgresqls' "$rbac"
grep -q 'cluster-agent-dataplane' "$rbac"
kubectl kustomize "$runtime" >/dev/null
