...
PostgreSQL Cluster 显式生成受控 superuser Secret，仅供 Langfuse bootstrap Job 幂等创建独立 role/database；应用 Pod 不读取该 Secret。MinIO 现有 RAG bucket 与 root 凭据保持不变，Langfuse bootstrap 创建专用用户，并只授权 `langfuse-events`、`langfuse-media`、`langfuse-exports`。两个 bootstrap Job 分别使用 `postgresBootstrapRevision`、`minioBootstrapRevision`，对应模板发生变化时只提升相关参数，避免 Kubernetes 不可变字段阻断新 Release 或无意义重跑另一侧。Web/Worker 和 bootstrap 均关闭 Service Links，避免 `REDIS_PORT=tcp://...` 等注入变量污染 Langfuse 配置。Chart 基线和验证入口位于 `tests/langfuse-chart-1.5.39-values.yaml`、`tests/verify-langfuse-chart-baseline.sh`，只包含虚构 Secret 引用。
