...
本目录保存 Agent Platform 在 OpenChoreo 控制平面的 namespaced 资源，包括 `development` Environment、`development-only` DeploymentPipeline、`agent-platform` Project，以及 MinIO、RabbitMQ、Milvus 三个共享资源申请。
...
MinIO 密码只由 OpenBao 的 `openchoreo/agent-platform/development/minio` 提供，并通过 External Secrets Operator 同步到数据平面。本目录不得写入密码、Token、kubeconfig 或 Secret 明文。
