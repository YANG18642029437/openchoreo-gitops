...
这组入口通过 MetalLB 固定地址 `192.168.2.154` 提供 HTTPS。Mac 的 OpenVPN 地址是
`10.8.0.10`，经过 OpenVPN 网关 SNAT 后，集群看到的来源是 `192.168.1.108`，因此入口
只允许 `192.168.1.108/32`。原有域名入口继续保留。
...
3. OpenVPN 网关的 SNAT 地址发生变化后，必须从现有入口日志重新确认并把 `service.yaml` 改成新的单个 `/32`，禁止为了方便开放整个网段。
